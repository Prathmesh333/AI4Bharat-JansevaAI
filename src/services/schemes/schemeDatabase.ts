...
function normalizeSchemeLookupValue(value: string): string {
...
/**
 * Resolve a scheme identifier that may be a slug, an encoded scheme name,
 * or a human-readable name generated by the AI.
 */
export function resolveSchemeByIdentifier(identifier: string): CsvScheme | null {
    const rawIdentifier = safeDecodeURIComponent((identifier || '').trim());
    if (!rawIdentifier) {
        return null;
    }

    const bySlug = getSchemeBySlug(rawIdentifier);
    if (bySlug) {
        return bySlug;
    }

    const normalizedIdentifier = normalizeSchemeLookupValue(rawIdentifier);
    if (!normalizedIdentifier) {
        return null;
    }

    const exactNameMatch = schemes.find(scheme =>
        normalizeSchemeLookupValue(scheme.scheme_name) === normalizedIdentifier
    );
    if (exactNameMatch) {
    ...
    if (normalizedIdentifier.length >= 8) {
        const partialNameMatch = schemes.find(scheme => {
            const normalizedSchemeName = normalizeSchemeLookupValue(scheme.scheme_name);
            return normalizedSchemeName.includes(normalizedIdentifier) ||
                normalizedIdentifier.includes(normalizedSchemeName);
        });
        ...
        return searchSchemesByText(rawIdentifier, 1)[0] || null;
    }
